...
export default {
  ...
  methods: {
    ...
    async open() {
      ...
      // 好想换成自己的微信收款码[坏笑]
      // 注意不能写成`<img src=${imgUrl}/>`，/前面应该要有空格，与src值隔开
      this.$alert(
        `<img src=${imgUrl} /><p style="color:red;">若支付遇到问题，请联系客服xxxx</p>`,
        "请微信扫码支付",
        {
          ...
          showClose: false, // 关闭右上角的x按钮
          beforeClose: (action, instance, done) => {
            // console.log(action);
            // console.log(instance);
            clearInterval(this.timer); // 关闭二维码messageBox时记得清除定时器
            this.timer=null
            done();
          },
        }
      ...
      if (!this.timer) {
        // 轮询，每隔1s向服务器发送请求，询问服务器用户是否支付成功
        this.timer = setInterval(async () => {
          // console.log(this); 注意定时器中的回调函数应使用箭头函数，否则会导致这里的this为window对象，这样就拿不到接口函数了
          let result = await this.$API.reqPayStatus(this.orderId);
          console.log(result);
          // if (result.code == 200) {
            clearInterval(this.timer);
            this.timer=null
            // $msgbox有一个关闭messageBox的API
            this.$msgbox.close()
