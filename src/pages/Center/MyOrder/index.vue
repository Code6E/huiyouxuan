  ...
  <div class="order-right">
    <div class="order-content">
        ...
        <table class="order-item" v-for="order in orderList" :key="order.id">
          ...
          <tbody>
            <tr v-for="(good,index) in order.orderDetailList" :key="good.id">
              ...
              <!-- 由于这里是遍历order.orderDetailList来显示tr，所以会导致本来想合并的单元格有多个，这里用v-if="index==0"控制只显示一个 -->
      ...
      <div class="choose-order">
        <Pagination :total="orderInfo.total" :pageSize="limit" :continues='5' :pageNo="page" @paginationEvent="paginationEvent"/>
      </div>
    </div>
    <!--猜你喜欢-->
    <div class="like">
...
export default {
  ...
  methods: {
    ...
    // 点击分页器按钮（自定义事件）的回调，pageNo是点击的按钮对应的页号，这个在组件触发自定义事件时会传递该参数，所以该参数不要在绑定自定义事件时（@paginationEvent="paginationEvent"）体现
