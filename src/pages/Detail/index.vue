          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <!-- 服务器返回的属性对象包含isChecked属性值，为1表示旋转，即添加avtive类，为0则没有active -->
                <!-- 因为服务器返回的isChecked是字符串类型，转换成布尔值总是true，应将其转换成数字类型 -->
                <dd
                  changepirce="0"
                  :class="{ active: Number(attr.isChecked) }"
...
export default {
  ...
  computed: {
    ...
    ...mapState({}),
    // 考虑到当服务器请求的数据还没拿到时，this.skuInfo是空对象（detail.js中定义的），此时this.skuInfo.skuImageList为undefined，直接读取this.skuInfo.skuImageList会报错（即使这种报错不影响程序运行，但尽量要去解决），可以利用短路让this.skuInfo.skuImageList为空数组。
    imgList() {
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    // 点击属性标签，给相应的属性标签添加active类
    changeActive(target, arr) {
      // 先将全部属性的isChecked置0，然后再将被点击的属性置1
    ...
    changeSkuNum(skuNum){
      // 字符串乘以1的结果是NaN, false*1=0, ture*1=1
      console.log(skuNum*1);
      // 输入字符串或小于1的数时，将其置1
    ...
    async addShopcart(){
      // 
      try{
        // 这里action中的函数addShopcart返回的是Promise对象
        let result=await this.$store.dispatch('addShopcart',{skuId:this.$route.params.id,skuNum:this.skuNum})
        // 因为对象类型的数据无法通过路由的query参数传递，所以这里用sessionStorage存储该参数，因为该参数是随着用户的购物行为临时产生的，不需要在关闭标签页后继续存储，所以不用localStorage。
        // 因为WebStorage的值会自动转换成字符串，所以应用JSON.stringify(this.skuInfo)将skuInfo转换成JSON字符串形式存储（用的时候再转换回来就好啦）。如果不转成JSON字符串的话，skuInfo会自动转成字符串"[object Object]"
  ...
  mounted() {
    this.$store.dispatch("goodsInfo", this.$route.params.id);
  },
};
</script>
